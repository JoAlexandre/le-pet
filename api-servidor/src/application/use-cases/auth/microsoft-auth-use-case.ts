...
export class MicrosoftAuthUseCase {
  constructor(
    ...
    private microsoftOAuthProvider: OAuthProvider,
  ...
  async execute(dto: OAuthLoginDto): Promise<AuthResponseDto> {
    let oauthData;
    try {
      oauthData = await this.microsoftOAuthProvider.validateToken(dto.idToken);
    } catch {
    ...
    if (existingUser) {
      if (existingUser.authProvider !== AuthProvider.MICROSOFT) {

...
export class GoogleWebAuthUseCase {
  constructor(
  ...
  generateAuthUrl(): string {
    if (!this.googleOAuthProvider.generateAuthUrl) {
      throw new Error('Google OAuth provider does not support web auth');
    }
    return this.googleOAuthProvider.generateAuthUrl();
  }

  async handleCallback(code: string): Promise<AuthResponseDto> {
    if (!this.googleOAuthProvider.exchangeCode) {
      throw new Error('Google OAuth provider does not support code exchange');
    }

    const oauthData = await this.googleOAuthProvider.exchangeCode(code);
